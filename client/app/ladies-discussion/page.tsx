...
export default function LadiesDiscussionPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !localStorage.getItem("gmsa_token")) {
      setLoading(false);
      return;
    }
    api<{ messages: DiscussionMessage[] }>("/wocom/discussion")
      ...
      .catch((e) => {
        const msg = getSafeErrorMessage(e, "Failed to load.");
        setError(msg);
        if (msg.includes("Ladies-only") || msg.includes("403") || msg.includes("Insufficient")) setAccessDenied(true);
      })
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-2xl font-bold text-gmsa-green mb-4">Ladies discussion</h1>
  ...
  if (typeof window !== "undefined" && !localStorage.getItem("gmsa_token")) {
    return (
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-2xl font-bold text-gmsa-green mb-4">Ladies discussion</h1>
        <p className="text-gray-600 mb-4">Please sign in to access the ladies-only discussion.</p>
        <button type="button" onClick={() => router.push("/login")} className="btn-primary">Sign in</button>
      </div>
    );
  }
  ...
  if (accessDenied) {
    return (
      <div className="container mx-auto px-4 py-12">
        <h1 className="text-2xl font-bold text-gmsa-green mb-4">Ladies discussion</h1>
        <p className="text-gray-600">This space is for ladies only. Access is restricted to female members and WOCOM.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gmsa-green mb-2">Ladies discussion</h1>
      <p className="text-gray-600 mb-4">Private chat for ladies. Messages are visible only to women members and WOCOM.</p>
      ...
      <div className="card overflow-hidden flex flex-col max-w-3xl mx-auto" style={{ minHeight: "400px" }}>
