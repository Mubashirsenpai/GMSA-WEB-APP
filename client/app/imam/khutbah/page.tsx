...
export default function ImamKhutbahPage() {
  ...
  const load = () => api<K[]>("/khutbah").then(setList).catch(() => setList([]));
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (addDate) form.append("date", addDate);
    form.append("file", addFile);
    const token = getToken();
    try {
      const res = await fetch(`${API_BASE}/khutbah`, {
        method: "POST",
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-6">Add Khutbah materials here. They appear on the public Downloads page.</p>

      <form onSubmit={handleAdd} className="card p-6 mb-6 max-w-xl">
        <h2 className="font-semibold text-gray-900 mb-4">Add Khutbah material</h2>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="date" value={addDate} onChange={(e) => setAddDate(e.target.value)} className="w-full border rounded-lg px-3 py-2" />
          </div>
          <button type="submit" disabled={addLoading} className="btn-primary">
            {addLoading ? "Uploading..." : "Add Khutbah material"}
          </button>
        </div>
      </form>
