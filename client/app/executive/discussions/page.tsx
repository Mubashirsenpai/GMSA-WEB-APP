...
function getSocketUrl(): string {
  ...
  // In local dev, Next may run on 3000, 3001, etc.; socket.io is always on the backend (4000)
  if (window.location.hostname === "localhost") return "http://localhost:4000";
  return window.location.origin;
}

export default function ExecutiveDiscussionsPage() {
  ...
  useEffect(() => {
    api<{ messages: DiscussionMessage[] }>("/executive/discussions")
  ...
  useEffect(() => {
    ...
    socket.on("new_message", (msg: DiscussionMessage) => {
      setMessages((prev) => {
    ...
    return () => {
      socket.off("connect").off("disconnect").off("new_message");
      socket.disconnect();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const created = await api<DiscussionMessage>("/executive/discussions", {
        method: "POST",
  ...
  if (loading) {
    return (
      <div>
        <h1 className="text-2xl font-bold text-gmsa-green mb-2">Live discussions</h1>
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-4">
        Real-time text chat for executive board members. Messages are visible only to executives and admins.
      </p>
      <div className="flex items-center gap-2 mb-3">
        <span
          className={`inline-block w-2 h-2 rounded-full ${connected ? "bg-green-500" : "bg-gray-400"}`}
          aria-hidden
        />
      ...
      <div className="card overflow-hidden flex flex-col" style={{ minHeight: "400px" }}>
        <div
          ref={listRef}
          className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[320px] max-h-[60vh]"
        >
          {messages.length === 0 && (
          ...
          {messages.map((m) => (
            <div key={m.id} className="flex flex-col">
              <div className="flex items-baseline gap-2 flex-wrap">
                ...
                {m.author.position && (
                  <span className="text-xs text-gmsa-green font-medium">{m.author.position}</span>
                )}
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-gray-100 bg-gray-50/50">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="btn-primary self-end shrink-0 px-4 py-2"
            >
